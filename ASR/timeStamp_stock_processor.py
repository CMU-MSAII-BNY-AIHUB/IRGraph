...
class TimeStampStockProcessor():
    ...
    def load_stock_data(self, stock_folder, xml_file):
        """
        ...
        stock_csv = os.path.join(stock_folder, xml_file.replace("xml", "csv"))
        sp500_csv = os.path.join(stock_folder, xml_file.replace(".xml", "-SP500.csv"))
        bkx_csv = os.path.join(stock_folder, xml_file.replace(".xml", "-KBW.csv"))

        try:
            # Load the data from the CSV files into pandas DataFrames
            stock_data = pd.read_csv(stock_csv)
            SP500_data = pd.read_csv(sp500_csv)
            BKX_data = pd.read_csv(bkx_csv)
        except FileNotFoundError:
            # If files are not found, call get_data function to retrieve default data
            print(f"File(s) not found: {xml_file}. Loading default data using get_stock_data().")
    ...
    def process_file(self, audio_path,audio_file, stock_folder, xml_path, xml_file, has_stock_data):
        ...
        print(time, ticker)
        if has_stock_data:
            stock_data, SP500_data, BKX_data = self.load_stock_data(stock_folder, xml_file)
        ...
        df = self.create_and_sort_dataframe(self.global_time, self.global_price)
